...
function getFilterParams($conn) {
    ...
    $filters = [
        ...
        'sort_by' => in_array($_GET['sort_by'] ?? '', ['name', 'stock_quantity', 'expiry_date', 'created_at']) ? 
                    $conn->real_escape_string($_GET['sort_by']) : 'name',
 ...
 */
function buildWhereClause($filters, &$params, &$types) {
    $where = [];
...
try {
    ...
    // Add base condition for restricted medicines
    $where[] = "m.`Restricted_Medicine` = 1";
    ...
    while ($row = $result->fetch_assoc()) {
        $medicines[] = [
            ...
            'Restricted_Medicine' => (int)$row['restricted_medicine'], // For backward compatibility
