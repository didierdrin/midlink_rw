	...
	<head>
		...
		<style>
			...
			@media print {
				...
				.invoice-box { box-shadow: none; border: 0; margin: 0; }
				body { margin: 0; }
			}
		</style>
	</head>
...
foreach ($result as $row) { //print_r($row);exit;
             ...
             ?>
	<body>
		<div class="no-print" style="max-width:800px;margin:10px auto 0;display:flex;gap:10px;justify-content:flex-end;">
			<button onclick="window.print()" style="padding:6px 12px;">Print</button>
			<button onclick="window.history.back()" style="padding:6px 12px;">Back</button>
		</div>
...
<td><?php
				...
				<?php $totalAmount += (float)$row2['total']; ?>
				<tr>
